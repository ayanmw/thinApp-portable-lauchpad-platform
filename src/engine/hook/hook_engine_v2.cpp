/**
 * @file hook_engine.cpp (V2 - 标准 Windows API 版本)
 * @brief Hook 引擎模块源文件
 * 
 * 基于 MinHook 实现 API Hook，用于拦截文件系统、注册表和进程创建相关 API。
 * 使用标准 Windows API (CreateFileW, RegCreateKeyW, CreateProcessW) 而非 Native API，
 * 避免依赖 winternl.h，提高代码稳定性和可移植性。
 * 
...
// Windows API 头文件 (标准，无需 winternl.h)
#include <Windows.h>
#include <winreg.h>  // 注册表 API
...
// 原函数指针保存（用于调用原始函数）
static decltype(&CreateFileW) OriginalCreateFileW = NULL;
static decltype(&RegCreateKeyW) OriginalRegCreateKeyW = NULL;
static decltype(&RegSetValueExW) OriginalRegSetValueExW = NULL;
static decltype(&CreateProcessW) OriginalCreateProcessW = NULL;
...
// 注册表 Hook 函数声明
static LSTATUS WINAPI HookedRegCreateKeyW(
    HKEY hKey,
    LPCWSTR lpSubKey,
    PHKEY phkResult
);
...
BOOL HookEngine_InstallHooks(void)
{
    ...
    // 安装文件系统 Hook (CreateFileW)
    {
        void* targetAddr = (void*)GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "CreateFileW");
        if (targetAddr == NULL) {
            printf("[HookEngine] 错误: 找不到 CreateFileW\n");
            return FALSE;
        }
        
        MH_STATUS status = MH_CreateHook(targetAddr, &HookedCreateFileW, (void**)&OriginalCreateFileW);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 创建 CreateFileW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        status = MH_EnableHook(targetAddr);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 启用 CreateFileW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        printf("[HookEngine] CreateFileW Hook 安装成功\n");
    }
    
    // 安装注册表 Hook (RegCreateKeyW, RegSetValueExW)
    {
        void* targetAddr1 = (void*)GetProcAddress(GetModuleHandleW(L"advapi32.dll"), "RegCreateKeyW");
        void* targetAddr2 = (void*)GetProcAddress(GetModuleHandleW(L"advapi32.dll"), "RegSetValueExW");
        
        if (targetAddr1 == NULL || targetAddr2 == NULL) {
            printf("[HookEngine] 错误: 找不到注册表 API\n");
            return FALSE;
        }
        
        MH_STATUS status = MH_CreateHook(targetAddr1, &HookedRegCreateKeyW, (void**)&OriginalRegCreateKeyW);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 创建 RegCreateKeyW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        status = MH_CreateHook(targetAddr2, &HookedRegSetValueExW, (void**)&OriginalRegSetValueExW);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 创建 RegSetValueExW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        status = MH_EnableHook(targetAddr1);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 启用 RegCreateKeyW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        status = MH_EnableHook(targetAddr2);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 启用 RegSetValueExW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        printf("[HookEngine] 注册表 Hook 安装成功\n");
    }
    
    // 安装进程创建 Hook (CreateProcessW)
    {
        void* targetAddr = (void*)GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "CreateProcessW");
        if (targetAddr == NULL) {
            printf("[HookEngine] 错误: 找不到 CreateProcessW\n");
            return FALSE;
        }
        
        MH_STATUS status = MH_CreateHook(targetAddr, &HookedCreateProcessW, (void**)&OriginalCreateProcessW);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 创建 CreateProcessW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        status = MH_EnableHook(targetAddr);
        if (status != MH_OK) {
            printf("[HookEngine] 错误: 启用 CreateProcessW Hook 失败 (0x%X)\n", status);
            return FALSE;
        }
        
        printf("[HookEngine] CreateProcessW Hook 安装成功\n");
    }
    
    printf("[HookEngine] 所有 Hook 安装完成\n");
...
BOOL HookEngine_UninstallHooks(void)
{
    ...
    if (status != MH_OK) {
        printf("[HookEngine] 警告: 卸载 Hook 失败 (0x%X)\n", status);
    }
...
/**
 * @brief Hook CreateFileW
 * 
 * 拦截文件创建/打开操作，重定向到虚拟文件系统（未来实现）。
 * 当前为桩实现，仅记录捕获事件。
 */
static HANDLE WINAPI HookedCreateFileW(
    LPCWSTR lpFileName,
    ...
    printf("[Hook] CreateFileW 被调用: %ls\n", lpFileName);
    
    // 记录捕获事件（若捕获模式已启用）
    if (g_CaptureModeEnabled && lpFileName != NULL) {
        RecordCaptureEvent(
...
/**
 * @brief Hook RegCreateKeyW
 * 
 * 拦截注册表键创建操作，重定向到虚拟注册表（未来实现）。
 * 当前为桩实现，仅记录捕获事件。
 */
static LSTATUS WINAPI HookedRegCreateKeyW(
    HKEY hKey,
    ...
    printf("[Hook] RegCreateKeyW 被调用\n");
    
    // 记录捕获事件（若捕获模式已启用）
    if (g_CaptureModeEnabled && lpSubKey != NULL) {
        WCHAR fullPath[MAX_PATH];
        wsprintfW(fullPath, L"HKEY_%p\\%ls", hKey, lpSubKey);
    ...
    // 调用原函数（透传）
    if (OriginalRegCreateKeyW != NULL) {
        return OriginalRegCreateKeyW(hKey, lpSubKey, phkResult);
    }
...
/**
 * @brief Hook RegSetValueExW
 * 
 * 拦截注册表值设置操作，重定向到虚拟注册表（未来实现）。
 * 当前为桩实现，仅记录捕获事件。
 */
static LSTATUS WINAPI HookedRegSetValueExW(
    HKEY hKey,
    ...
    printf("[Hook] RegSetValueExW 被调用\n");
    
    // 记录捕获事件（若捕获模式已启用）
    if (g_CaptureModeEnabled && lpValueName != NULL) {
        WCHAR fullPath[MAX_PATH];
        wsprintfW(fullPath, L"HKEY_%p\\%ls", hKey, lpValueName);
        
        RecordCaptureEvent(
            CAPTURE_EVENT_REG_SET_VALUE,
            fullPath,
            lpData,
            cbData
        );
    ...
    // TODO: 重定向注册表值到虚拟 hive
    
    // 调用原函数（透传）
    if (OriginalRegSetValueExW != NULL) {
...
/**
 * @brief Hook CreateProcessW
 * 
 * 拦截进程创建操作，注入 Hook DLL 到子进程（未来实现）。
 * 当前为桩实现，仅记录捕获事件。
 */
static BOOL WINAPI HookedCreateProcessW(
    LPCWSTR lpApplicationName,
    ...
    printf("[Hook] CreateProcessW 被调用: %ls\n", lpApplicationName);
